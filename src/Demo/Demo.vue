  ...
  <div class="main">
    ...
    <div class="bar">
      ...
      <div style='display: flex; gap: 5px'>
        <!-- <button @click="openFileDialog">파일 열기</button> -->
          <input type="file" ref="fileInput" @change="handleFileChange" style="display: none;" />
        <!-- <button @click="showSaveDialog = true">json 파일로 다운로드</button> -->
        <div v-if="showSaveDialog">
          <input type="text" v-model="fileName" placeholder="파일 이름을 입력하세요" />
...
export default {
  ...
  data () {
    // 반복시킬 Lorem Ipsum 문단
      const lorem = "<p>What is Lorem Ipsum?Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s ... (이하 생략). There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary, making this the first true generator on the Internet. It uses a dictionary of over 200 Latin words, combined with a handful of model sentence structures, to generate Lorem Ipsum which looks reasonable. The generated Lorem Ipsum is therefore always free from repetition, injected humour, or non-characteristic words etc.</p>";

      // content 배열을 for문으로 생성
      let contentArr="";
      for (let i = 0; i < 50; i++) {   // 3번 반복 (원하는 만큼 조절)
        contentArr +=lorem;
      }
    return {
      ...
      content: [contentArr],
      fileName: '', // 저장할 파일 이름 
      showSaveDialog: false,  // 파일 이름 입력 창 표시 여부
      zoom: 1.0,
      zoom_min: 0.10,
      zoom_max: 5.0,
  ...
  computed: {
    ...
    menu () {
      return [
        // Main commands
        { text: "새문서", title: "새문서", icon: "description", click: () => { if(confirm("This will create an empty document. Are you sure?")){ this.content = [""]; this.resetContentHistory(); } } },
        { text: "열기", title: "열기", icon: "description", click: () => { this.$refs.fileInput.click() }},
        { text: "프린트", title: "프린트", icon: "print", click: () => window.print() },
        { text: "다운로드", title: "다운로드", icon: "description", click: () => {
          if (this.showSaveDialog) {this.showSaveDialog=false}
          else this.showSaveDialog=true }
        },
  ...
  methods: {
    ...
    savePropsToFile() { 
      ...
      //const data = { message: this.content, count: this.content_idx };
      const json = JSON.stringify(this.content); 
      const blob = new Blob([json], { type: 'application/json' }); 
      const url = URL.createObjectURL(blob); 
      const link = document.createElement('a');
      link.href = url; 
      //link.download = 'propsData.json';
      link.download = this.fileName.endsWith('.json') ? this.fileName : `${this.fileName}.json`; // 확장자 확인      
    ...
    handleFileChange(event) {
      ...
      reader.onload = (e) => {
        try {
          ...
          this.content = JSON.parse(e.target.result);
          // update document
          // call renderer
